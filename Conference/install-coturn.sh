...
# Аутентификация через статический пользователь
# Используем lt-cred-mech для поддержки username/password
lt-cred-mech
no-cli

# Статический пользователь для TURN (username: turnuser, password: turnpass)
# Этот пользователь будет использоваться всеми клиентами
...
# Безопасность
fingerprint
# lt-cred-mech несовместим с no-auth, убираем
...
# Создаем пользователя для TURN через turnadmin (альтернативный способ)
# Но мы используем статический user= в конфиге, так что это не нужно
